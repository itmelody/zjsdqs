  ...
  <div class="third-party-page">
    ...
    <a-modal v-model:open="modalVisible" :title="modalTitle" width="720px" :footer="formMode === 'view' ? null : undefined" @ok="submitForm" @cancel="modalVisible = false">
      <a-form :label-col="{ span: 8 }" :wrapper-col="{ span: 16 }">
        <a-row :gutter="16">
          <a-col :span="12">
            <a-form-item label="单位名称" required><a-input v-model:value="formData.name" placeholder="请输入企业全称" :disabled="formMode === 'view'" /></a-form-item>
          </a-col>
          <a-col :span="12">
            <a-form-item label="统一社会信用代码" required><a-input v-model:value="formData.creditCode" placeholder="请输入统一社会信用代码" :disabled="formMode === 'view'" /></a-form-item>
          </a-col>
          <a-col :span="12">
            <a-form-item label="归属地区" required>
              <a-select v-model:value="formData.region" placeholder="请选择地区" :disabled="formMode === 'view'" allowClear>
                <a-select-option v-for="opt in regionFullOptions" :key="opt" :value="opt">{{ opt }}</a-select-option>
              </a-select>
            </a-form-item>
          </a-col>
          <a-col :span="12">
            <a-form-item label="详细地址" required><a-input v-model:value="formData.address" placeholder="请输入单位详细地址" :disabled="formMode === 'view'" /></a-form-item>
          </a-col>
          <a-col :span="12">
            <a-form-item label="法定代表人" required><a-input v-model:value="formData.legalPerson" placeholder="请输入法定代表人" :disabled="formMode === 'view'" /></a-form-item>
          </a-col>
          <a-col :span="12">
            <a-form-item label="法定代表人联系方式" required><a-input v-model:value="formData.legalPhone" placeholder="请输入法定代表人联系方式" :disabled="formMode === 'view'" /></a-form-item>
          </a-col>
          <a-col :span="12">
            <a-form-item label="单位负责人" required><a-input v-model:value="formData.unitHead" placeholder="请输入单位负责人" :disabled="formMode === 'view'" /></a-form-item>
          </a-col>
          <a-col :span="12">
            <a-form-item label="单位负责人联系方式" required><a-input v-model:value="formData.headPhone" placeholder="请输入单位负责人联系方式" :disabled="formMode === 'view'" /></a-form-item>
          </a-col>
          <a-col :span="12">
            <a-form-item label="日常联系人" required><a-input v-model:value="formData.dailyContact" placeholder="请输入日常联系人" :disabled="formMode === 'view'" /></a-form-item>
          </a-col>
          <a-col :span="12">
            <a-form-item label="日常联系人联系方式" required><a-input v-model:value="formData.contactPhone" placeholder="请输入日常联系人联系方式" :disabled="formMode === 'view'" /></a-form-item>
          </a-col>
          <a-col :span="12">
            <a-form-item label="企业状态" required>
          ...
          <a-col :span="12">
            <a-form-item label="行业主管单位" required><a-input v-model:value="formData.supervisor" placeholder="请输入行业主管单位" :disabled="formMode === 'view'" /></a-form-item>
          </a-col>
          <a-col :span="24">
            <a-form-item label="备注" :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }"><a-textarea v-model:value="formData.remark" placeholder="请输入备注" :rows="3" :disabled="formMode === 'view'" /></a-form-item>
          </a-col>
        </a-row>
      </a-form>
...
const tableData = ref<UnitRow[]>([
  { id: 1, name: '越城区公用事业集团', creditCode: '91330602MA29D10B24', region: '绍兴市越城区', address: '浙江省绍兴市越城区皋埠街道吼山路55号', legalPerson: '杨烈勇', legalPhone: '+86-13606756967', unitHead: '胡清培', headPhone: '18957592123', dailyContact: '王永彬', contactPhone: '18989559822', status: '启用', supervisor: '越城区住建局', remark: '' },
  { id: 2, name: '花草养护公司', creditCode: '988111254441', region: '杭州市余杭区', address: '测试', legalPerson: '李一', legalPhone: '18419980001', unitHead: '李一二', headPhone: '18419980002', dailyContact: '李一五', contactPhone: '18419980003', status: '启用', supervisor: '余杭区住建局', remark: '' },
  { id: 3, name: '养护单位001', creditCode: 'ceshi1111', region: '宁波市海曙区', address: '详细地址', legalPerson: '法定代表人', legalPhone: '15666666666', unitHead: '单位责任人', headPhone: '15777777777', dailyContact: '日常联系人', contactPhone: '15888888888', status: '启用', supervisor: '海曙区住建局', remark: '' },
  { id: 4, name: '林木养护公司', creditCode: '991001002', region: '杭州市余杭区', address: '测试', legalPerson: '张三', legalPhone: '18419970001', unitHead: '李四', headPhone: '18419970002', dailyContact: '王五', contactPhone: '18419970003', status: '启用', supervisor: '余杭区住建局', remark: '' },
])
...
const formData = ref({
  name: '', creditCode: '', region: undefined as string | undefined, address: '',
  legalPerson: '', legalPhone: '', unitHead: '', headPhone: '',
  dailyContact: '', contactPhone: '', status: '启用', supervisor: '', remark: '',
})

const modalTitle = computed(() => {
  if (formMode.value === 'add') return '新增日常养护单位'
  if (formMode.value === 'view') return '查看日常养护单位'
  return '编辑日常养护单位'
})
...
function openModal(mode: 'add' | 'view' | 'edit', record?: UnitRow) {
  ...
  if (mode === 'add') {
    ...
    formData.value = { name: '', creditCode: '', region: undefined, address: '', legalPerson: '', legalPhone: '', unitHead: '', headPhone: '', dailyContact: '', contactPhone: '', status: '启用', supervisor: '', remark: '' }
  } else if (record) {
    editingId.value = record.id
    formData.value = { name: record.name, creditCode: record.creditCode, region: record.region, address: record.address, legalPerson: record.legalPerson, legalPhone: record.legalPhone, unitHead: record.unitHead, headPhone: record.headPhone, dailyContact: record.dailyContact, contactPhone: record.contactPhone, status: record.status, supervisor: record.supervisor, remark: record.remark }
  }
